...
def barplot_data(aData_in,\
    ...
    def autolabel(rects):
        ...
        for rect in rects:
            ...
            #sFormat_y
            #txt = "{sFormat_y}".format(height)
            txt = sFormat_y % height
            #txt = MyFormatter()
            #txt = txt.format(value = height)
            ax.annotate(txt, 
    ...
    for i in np.arange(1, nData + 1, 1):
        ...
        data1 = aData_in[i-1]
        if i == 1 or i ==2:
            rects1 = ax.bar( x - total_width * 0.5 + i * width, data1, width, label= aLabel_y_in[i-1],\
