...
def map_netcdf_file(
    ...
    try:
        ...
        if iFlag_unstructured_in is not None:
            ...
            if pProjection_map_in is not None:
                ...
                if pBoundary_in is None:
                    pProjection_map = cpl.crs.Orthographic(
                        central_longitude=0.50 * (dLongitude_min + dLongitude_max),
                        central_latitude=0.50 * (dLatitude_min + dLatitude_max),
                        globe=None,
                    )
                else:
                    ...
                    pProjection_map = cpl.crs.Orthographic(
                        central_longitude=centroid_x,
                        central_latitude=centroid_y,
                        globe=None,
                    )
            ...
            aLongitude_center = (
                np.arange(ncolumn) * dResolution_x + dLongitude_min + half_res_x
            )
            aLatitude_center = (
                np.arange(nrow) * dResolution_y + dLatitude_min + half_res_y
            )

            aLongitude_grid, aLatitude_grid = np.meshgrid(
                aLongitude_center, aLatitude_center
            )
                ...
                for iMonth in range(1, 13):
                    ...
                    for i in range(0, nrow):
                        for j in range(0, ncolumn):
                            ...
                            coords = [
                                (
                                    dLongitude_cell_center - half_res_x,
                                    dLatitude_cell_center - half_res_y,
                                ),
                                (
                                    dLongitude_cell_center + half_res_x,
                                    dLatitude_cell_center - half_res_y,
                    ...
                    if aExtent_in is None:
                        if pBoundary_in is None:
                            ...
                            if (
                                dLongitude_min - marginx < -180
                                or dLongitude_max + marginx > 180
                            ):
                                marginx = 0
                            if (
                                dLatitude_min - marginy < -90
                                or dLatitude_max + marginy > 90
                            ):
                                marginy = 0

                            aExtent = [
                                dLongitude_min - marginx,
                                dLongitude_max + marginx,
                                dLatitude_min - marginy,
                                dLatitude_max + marginy,
                            ]
                        else:
                            ...
                            aExtent = [
                                pBoundary_box[0] - marginx,
                                pBoundary_box[1] + marginx,
                                pBoundary_box[2] - marginy,
                                pBoundary_box[3] + marginy,
                            ]
                    else:
                        aExtent = aExtent_in
            ...
            if iFlag_daily == 1:
                ...
                color_indices = np.clip(
                    ((aData - dValue_min) / (dValue_max - dValue_min) * 255).astype(
                        int
                    ),
                    0,
                    255,
