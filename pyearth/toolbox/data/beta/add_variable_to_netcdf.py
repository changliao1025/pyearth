...
def add_variable_to_netcdf(
    sFilename_old, sFilename_new, aData_in, aVariable_in, aUnit_in, aaDimension_in
):
    ...
    for sKey, aValue in pDatasets_in.variables.items():
        ...
        outVar = pDatasets_out.createVariable(
            sKey, aValue.datatype, dummy, fill_value=-9999
        )
...
def add_multiple_variable_to_netcdf(
    sFilename_old, sFilename_new, aData_in, aVariable_in, aUnit_in, aDimension_in
):
    try:
        import netCDF4 as nc
    except ImportError as e:
    ...
    for iVariable in range(nVariable):
        ...
        pVar3 = pDatasets_out.createVariable(
            sVariable, "f4", aDimension_tuple, fill_value=missing_value
        )
        pVar3[:] = aData
