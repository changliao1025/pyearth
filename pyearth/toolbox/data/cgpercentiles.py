...
def cgpercentiles( aData_in, aPercentiles_in, missing_value_in=None ):
    ...
    nan_count1 = nan_index1[0].size#np.count_nonzero(aData_copy == min_value)
    nan_count2 = nan_index2[0].size#np.count_nonzero(aData_copy == max_value)
    ...
    if scenario ==1:
        ...
        if good_count <= num_per :
          ...
          sample_count1 =  np.ceil( float(good_count) / (num_per-1)) + 1 #why plus one?
                ...
                if scenario ==4:
                    ...
                    if scenario ==5:
                        ...
                        if good_count <= num_per :
                          ...
                          sample_count3 = np.floor( float(good_count) / (num_per-2)) - 1
