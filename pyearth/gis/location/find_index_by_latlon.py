def find_index_by_latlon(dLon_in, dLat_in, dLon_min_in, dLat_max_in, 
    ...
    if iFlag_center == 1: #when the max and min are center 
        ...
        iRow_index = round( ( (dLat_max_in - 0.5*dResolution_y_in)  - dLat_in ) /  dResolution_y_in )
        iColumn_index = round(( dLon_in - (dLon_min_in + 0.5*dResolution_x_in )  ) /  dResolution_x_in )
        pass
