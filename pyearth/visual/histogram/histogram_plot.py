...
def histogram_plot(
    ...
    if aPlot_order_in is None:
        ...
        if not (
            isinstance(aPlot_order_in, (list, np.ndarray))
            and len(aPlot_order_in) == nData
            and all(isinstance(idx, (int, np.integer)) for idx in aPlot_order_in)
            and sorted(list(set(aPlot_order_in))) == list(np.arange(nData))
        ):
            raise ValueError(
                f"aPlot_order_in must be a list or array of unique indices from 0 to {nData-1}, "
    ...
    for plot_sequence_idx, original_idx in enumerate(aPlot_order):
        ...
        if iFlag_normalize == 1:  # <-- Check normalization flag
            N, bins, hisp_patches = ax_histo.hist(
                aData_for_hist,
                bins=num_bins,
    ...
    if iFlag_density == 1:
        for plot_sequence_idx_density, original_idx_density in enumerate(aPlot_order):
            aData_kde = aData_all[
                original_idx_density
            ]  # Use original, unclipped data for KDE

            # Add a check for empty or single-value data to prevent KDE errors
            if len(aData_kde) == 0 or (
                len(aData_kde) > 0 and np.all(aData_kde == aData_kde[0])
            ):
            ...
            try:
                ...
                ax_histo.plot(
                    xx,
                    yy,
                    color=aColor[original_idx_density],
                    linestyle="--",
    ...
    for idx in range(nData):  # Iterate in original data order
        if (
            legend_patch_containers[idx] is not None
        ):  # Check if this dataset was plotted
