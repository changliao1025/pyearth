    ...
    for iax in range(len(ax_all)):
        ...
        if iax == 0:
            ...
            if nYear <= 3:
                ...
                ax.set_xticklabels(
                    [mpl.dates.num2date(x).strftime("%Y-%m") for x in ax.get_xticks()]
                )
            else:
                ax.xaxis.set_major_locator(pYear)
                ax.xaxis.set_minor_locator(pMonth)
            ...
            pass
            if iFlag_miniplot == 1:
                rect_bottom_left = ax.transData.transform(
                    (dMin_mini_x_num, dMin_mini_y)
                )
                ...
                rect_bottom_left_fig = fig.transFigure.inverted().transform(
                    rect_bottom_left
                )
                rect_top_right_fig = fig.transFigure.inverted().transform(
                    rect_top_right
                )
    ...
    if iFlag_miniplot == 1:
        ...
        arrow_left = FancyArrowPatch(
            (
                dMin_mini_x_fig,
                dMin_mini_y_fig,
            ),  # Start point (bottom-left corner of the rectangle)
        ...
        arrow_right = FancyArrowPatch(
            (
                dMax_mini_x_fig,
                dMin_mini_y_fig,
            ),  # Start point (bottom-right corner of the rectangle)
