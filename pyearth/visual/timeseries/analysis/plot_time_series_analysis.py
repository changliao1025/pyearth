...
def plot_time_series_analysis(
    ...
    try:
        ...
        raise ImportError(
            "The package 'pandas and statsmodels' is required for this function to run."
        ) from e
    ...
    if iFlag_without_raw == 1:
        ...
        fig, pAxGrid = plt.subplots(
            nrows=len(aData_all), figsize=(iSize_x, iSize_y), sharex=True, dpi=iDPI
        )
        ...
        for i, ax in enumerate(pAxGrid):
            ...
            if i == 2:
                ax.plot(
                    (dMin_x, dMax_x), (0, 0), color="#000000", linestyle=":", zorder=2
                )
