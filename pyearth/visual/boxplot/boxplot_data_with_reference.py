...
def boxplot_data_with_reference(
    aData_in,
    aLabel_x_in,
    sFilename_out,
    ...
    for i in np.arange(1, nData + 1, 1):
        ...
        for j in range(1, nData1 + 1, 1):
            ...
            x0 = [x_mid, x_mid]
            y0 = [q0_raw, q1_raw]
            (line,) = ax.plot(x0, y0, color="black", linestyle="--", linewidth=1)
            # draw short line
    ...
    if iFlag_ref == 1:
        for i in range(nData_reference):
            ...
            (line,) = ax.plot(
                x0,
                y0,
                color=aColor[i + nData1],
                linestyle="dashed",
                marker=aMarker_in[i + nData1],
                linewidth=1.5,
