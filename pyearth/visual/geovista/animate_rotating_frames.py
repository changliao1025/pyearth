...
def animate_rotating_frames(pMesh, aValid_cell_indices: np.ndarray, pConfig: VisualizationConfig, pConfig_anima: AnimationConfig,
                             sScalar: Optional[str] = None,
                             sUnit: Optional[str] = None,
                             sFilename_out: Optional[str] = None) -> bool:
    """
    ...
    dAmplitude_deg = pConfig_anima.amplitude_deg
    dCycles = pConfig_anima.cycles
    dPhase = pConfig_anima.phase
    # Initialize movie recording
    pPlotter.open_movie(sFilename_out, framerate=pConfig_anima.frames)
    if pConfig.verbose:
    ...
    for iFrame in range(pConfig_anima.frames):
        ...
        dLongitude_current = pConfig_anima.dLongitude_start + (iFrame * pConfig_anima.speed)
        ...
        dLatitude_current = float(pConfig_anima.dLatitude_start) + dAmplitude_deg * math.sin(dTheta)
