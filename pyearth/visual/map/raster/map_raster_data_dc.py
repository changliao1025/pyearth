...
def map_raster_data_dc(
    ...
    if iFlag_contour == 1:
        ...
        levels = cgpercentiles(
            aImage_in, aPercentiles_in, missing_value_in=dMissing_value
        )
    ...
    cmap = mpl.colors.ListedColormap(aColor[1 : ncolor - 1]).with_extremes(
        over=aColor[ncolor - 1], under=aColor[0]
    )
