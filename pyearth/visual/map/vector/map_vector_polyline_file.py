...
def map_vector_polyline_file(
    ...
    if iFlag_color == 1:
        ...
        if iFlag_discrete == 1:
            ...
            nValue_field = len(
                aValue_field_color
            )  # get unique values from the aValue_field_color

        if iFlag_data_min == 1:  # min is provided
            dValue_min = dData_min  # np.min(aValue_field_color)
        else:
            aValue_field_color = aValue_field_color[
                aValue_field_color != dMissing_value
            ]
        ...
        aValue_field_color = np.clip(
            aValue_field_color, a_min=dValue_min, a_max=dValue_max
        )
        ...
        if sGeometry_type == "LINESTRING":
            ...
            if iFlag_color == 1:
                if iFlag_discrete == 1:
                    ...
                    color_index = (dValue_color - dValue_min) / (
                        dValue_max - dValue_min
                    )
            ...
            if iFlag_thickness == 1:
                iThickness = remap(
                    dValue_thickness,
                    dValue_thickness_min,
                    dValue_thickness_max,
                    iThickness_min,
                    iThickness_max,
                )
            else:
    ...
    if iFlag_global_in is not None:
        ...
        if iFlag_zebra == 1:
            ax.set_xticks(
                np.arange(minx, maxx + (maxx - minx) / 11, (maxx - minx) / 10)
            )
            ax.set_yticks(
                np.arange(miny, maxy + (maxy - miny) / 11, (maxy - miny) / 10)
            )
            ax.set_axis_off()
