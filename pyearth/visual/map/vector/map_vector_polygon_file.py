...
def map_vector_polygon_file(
        ...
        if aDict_value_color_in is not None:
            ...
            for val_code in aValue:  # aValue is sorted unique values
                ...
                if color_for_val is None:
                    print(
                        f"Warning: Color for value {val_code} not found in dict_value_colors_in. Falling back to sColormap."
                    )
    ...
    try:
        ...
        if iBasemap_zoom_level_in is not None:
            ...
            iBasemap_zoom_level = calculate_zoom_level(
                scale_denominator, pProjection, dpi=int(iDPI / 2)
            )
        ...
        if iFlag_openstreetmap_in is not None and iFlag_openstreetmap_in == 1:
            ...
            if iFlag_terrain_image_in is not None and iFlag_terrain_image_in == 1:
                ...
                multi_poly = stamen_terrain.crs.project_geometry(
                    ll_target_domain, ccrs.PlateCarree()
                )
                target_domain = multi_poly.geoms[0]
                _, aExtent_terrain, _ = stamen_terrain.image_for_domain(
                    target_domain, iBasemap_zoom_level
                )
                img_stadia_terrain = Stadia_terrain_images(aExtent, iBasemap_zoom_level)
                ax.imshow(
                    img_stadia_terrain,
                    extent=aExtent_terrain,
                    transform=stamen_terrain.crs,
                )
                ...
                if (
                    iFlag_esri_hydro_image_in is not None
                    and iFlag_esri_hydro_image_in == 1
                ):
                    ...
                    ll_target_domain = sgeom.box(minx, miny, maxx, maxy)
                    multi_poly = esri_hydro.crs.project_geometry(
                        ll_target_domain, ccrs.PlateCarree()
                    )
                    target_domain = multi_poly.geoms[0]
                    _, aExtent_terrain, _ = esri_terrain.image_for_domain(
                        target_domain, iBasemap_zoom_level
                    )
                    _, aExtent_hydro, _ = esri_hydro.image_for_domain(
                        target_domain, iBasemap_zoom_level
                    )
                    if iFlag_relief_image_in is not None:
                        img_esri_terrain = Esri_relief_images(
                            aExtent, iBasemap_zoom_level
                        )
                    else:
                        img_esri_terrain = Esri_terrain_images(
                            aExtent, iBasemap_zoom_level
                        )
                    ...
                    ax.imshow(
                        img_esri_terrain,
                        extent=aExtent_terrain,
                        transform=esri_terrain.crs,
                    )
                    ax.imshow(
                        img_eari_hydro, extent=aExtent_hydro, transform=esri_hydro.crs
                    )
                    # add the license information
                    sLicense_info = "© Esri Hydro Reference Overlay"
                    sLicense_info_wrapped = "\n".join(
                        textwrap.wrap(sLicense_info, width=60)
                    )
                    ax.text(
                        0.5,
                        0.05,
            ...
            if iFlag_color == 1:
                ...
                if dValue != dMissing_value:
                    ...
                    if iFlag_discrete == 1:
                        ...
                        iValue = int(
                            dValue
                        )  # this maight cause issue if the date is not using integer
            ...
            if sGeometry_type == "MULTIPOLYGON":
                ...
                if sGeometry_type == "POLYGON":
                    if iFlag_buffer_in is not None:
                        aCoords_gcs = get_geometry_coordinates(
                            pGeometry_in.Buffer(1.0e-5)
                        )
                    else:
                        aCoords_gcs = get_geometry_coordinates(pGeometry_in)
                    aCoords_gcs = aCoords_gcs[:, 0:2]
    ...
    if iFlag_colorbar == 1:
        ...
        if iFlag_scientific_notation_colorbar == 1:
            ...
            if iFlag_discrete == 1:
                ...
                current_extend = (
                    sExtend if not iFlag_discrete else "neither"
                )  # Or None, or handle based on actual data clipping
                ...
                if len(aValue) > 0:  # Ensure aValue is not empty
                    for k_idx in range(len(aValue)):  # Iterate for each category/color
                        tick_centers.append(
                            (boundaries[k_idx] + boundaries[k_idx + 1]) / 2.0
                        )
                ...
                if aDict_discrete_labels_in is not None and len(aValue) > 0:
                    labels = [
                        aDict_discrete_labels_in.get(int(tick_val), str(int(tick_val)))
                        for tick_val in aValue
                    ]
                    cb.set_ticklabels(labels)
                    cb.ax.tick_params(
                        axis="y", which="major", labelsize=discrete_tick_labelsize
                    )  # Apply specific size
                elif len(aValue) > 0:
