...
def plot_study_area(
    ...
    try:
        ...
        raise ImportError(
            "The package 'requests' is required for this function to run."
        ) from e
    ...
    dem_proj = cpl.crs.AlbersEqualAre(
        central_longitude=pSpatialRef_source.GetProjPar("longitude_of_center"),
        central_latitude=pSpatialRef_source.GetProjPar("latitude_of_center"),
        standard_parallels=(
            pSpatialRef_source.GetProjPar("standard_parallel_1"),
    ...
    if sFilename_polyline_in is None:
        ...
        for pFeature in pLayer:
            ...
            if sGeometry_type == "LINESTRING":
                ...
                (line,) = ax_dem.plot(x, y, color="b", transform=cpl.crs.PlateCarree())
                lID = lID + 1
    ...
    uv_xcenter, uv_ycenter = reproject_coordinates(
        dLongitude_center,
        dLatitude_center,
        pSpatialRef_source.ExportToWkt(),
        ge_proj.to_wkt(),
    )
    xsize_ge = 600
    ysize_ge = 600
    ...
    xrange = (
        uv_xcenter - (xsize_ge / 2.0 * scale),
        uv_xcenter + (xsize_ge / 20 * scale),
    )
    yrange = (
        uv_ycenter - (ysize_ge / 2.0 * scale),
    ...
    if sFilename_slope_in is None:
        ...
        dSpace_x = 4
        ax_histo.hist(
            aSlope, int((dMax_x - dMin_x) / dSpace_x), color="skyblue", ec="skyblue"
        )
